...
pub fn part_1(a: &mut Vec<isize>, b: &mut Vec<isize>) -> isize {
    a.sort();
    b.sort();
    let mut sum: isize = 0;
...
pub fn part_2(a: &mut Vec<isize>, b: &mut Vec<isize>) -> isize {
    let mut frequencies: FxHashMap<isize, isize> = FxHashMap::default();
    for b in b.iter() {
        *frequencies.entry(*b).or_insert(0) += 1;
    }
    let sim_score: isize = a.iter().map(|x| if let Some(v) = frequencies.get(x) {
        x * v
    } else { 0 } ).sum();
    sim_score
}
